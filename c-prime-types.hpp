...
#pragma once
// A named guard in addition to `#pragma once`: the transpiler may drop a copy of
// this header next to the generated code, so it can be reached through more than
// one path in a single translation unit. The macro dedupes by content identity.
